...
public class ElementResolver {
	...
	public static EntityElement resolve(Class<?> persistentClass){
		...
		Set<Field> fields =  EasyJdbcHelper.getFields(persistentClass, mappedSuperclass);
		for(Field field:fields){
			if(Modifier.isFinal(field.getModifiers())) {
				continue;
			}
			String fieldName = field.getName();
			Method readMethod = EasyJdbcHelper.getReadMethod(persistentClass, mappedSuperclass, fieldName);
			Method writeMethod = EasyJdbcHelper.getWriteMethod(persistentClass,mappedSuperclass, fieldName, field.getType());
			...
			if(StringUtil.isEmpty(fieldElement.getColumn())) {
				fieldElement.setColumn(EasyJdbcHelper.camelToUnderline(fieldName));
			}
	...
	public static DynamicEntityElement resolveDynamic(Class<?> dynamicEntityClass,Map<String,String> dynamicMappings) {
		...
		Set<Field> fields = EasyJdbcHelper.getFields(dynamicEntityClass,superclass);
		for(Field field:fields){
			if(Modifier.isFinal(field.getModifiers())) {
				continue;
			}
			String fieldName = field.getName();
			Method readMethod = EasyJdbcHelper.getReadMethod(dynamicEntityClass,superclass, fieldName);
			Method writeMethod = EasyJdbcHelper.getWriteMethod(dynamicEntityClass, superclass, fieldName, field.getType());
			...
			String columnName =  EasyJdbcHelper.camelToUnderline(fieldName);
